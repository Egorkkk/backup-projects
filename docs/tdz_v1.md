...
Изменение UI не должно менять логику inventory.
Изменение policy не должно требовать переписывания DB-слоя.
Изменение restic adapter не должно ломать inventory scan.
...
CLI — обязательный служебный интерфейс.
Через CLI должны быть доступны все ключевые операции.
...
Autosave не должен исключаться автоматически только по имени.
Если autosave соответствует allowed extensions и не попадает под исключение cache/misc, он должен бэкапиться.
