...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Enable one manual include row in SQLite.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        with session_scope(session_factory) as session:
            ...
            if manual_include.enabled:
                print(f"Manual include already enabled: {manual_include.id}")
                return 0
        ...
        print(f"Enabled manual include: {args.manual_include_id}")
