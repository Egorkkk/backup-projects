...
from backup_projects.adapters.db.schema import excluded_patterns, extension_rules, settings
...
class RulesRepository:
    ...
    def create_extension_rule(
        ...
        *,
        ...
        record = self._session.execute(
            select(extension_rules).where(extension_rules.c.id == rule_id)
        ).mappings().one()
    ...
    def list_excluded_patterns(
        self, *, enabled_only: bool = False
    ) -> list[ExcludedPatternRecord]:
    ...
    def get_setting(self, key: str) -> SettingRecord | None:
        row = self._session.execute(
            select(settings).where(settings.c.key == key)
        ).mappings().one_or_none()
        if row is None:
            return None
